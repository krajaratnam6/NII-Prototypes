...
class LeveledAgentWithFeedback():
    ...
    def chat_to_agent(self, user_input, testing=False, ending=False):
        ...
        best_response = ""
        while iter < self.max_iter and best_score[0] < self.min_level_ratio:
            response = chat(
                llm_model,
            ...
            if self.verbosity and not testing:
                log(self.file, f"\n\n***Leveling Iteration {iter+1} of {self.max_iter}***\n\n")
                log(self.file, f"Potential response: '{response.message.content}'\n\n")
                log(self.file, f"Sentence-level CEFR predictions: {[cefr_levels[p] for p in preds]}.\n")
                log(self.file, f"On-level score: {ratio}, Desired score: {self.min_level_ratio}.\n")

            if ratio >= self.min_level_ratio:
                break
            ...
            if self.verbosity and not testing:
                log(self.file, f"Generated Feedback: '{feedback.message.content}'\n\n")
        ...
        if not testing:
            ...
            log(self.file, f"\nSending Response: '{best_response}'\n")
